...
class Register:
    def __init__(self, root):
        ...
        left = Label(self.root, image=self.left).place(
            x=1, y=100, width=515, height=570)

        # Register frame

        frame1 = Frame(self.root, bg="white")
        frame1.place(x=515, y=105, width=795, height=550)
        ...
        # self.btn_img=ImageTk.PhotoImage(file="rg2.png")
        btn_register = Button(frame1, text="REGISTER", font=("times new roman", 25, "bold"),bg="white", fg="green", bd=0, cursor="hand2",command=self.register_data).place(x=250, y=450)

        btn_login = Button(self.root, text="Sign In",command=self.login_window, bg="green", font=("times new roman", 25, "bold"), bd=0, cursor="hand2").place(x=160, y=650, width=200)
        ...
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="" :
            ...
            try:
                ...
                if row!=None:
                    ...
                    cur.execute("insert into studentregister (f_name,l_name,contact,email,question,answer,password) values(?,?,?,?,?,?,?)",
                                        (self.txt_fname.get(),
                                        self.txt_lname.get(),
